                ...
                if (e._isVisible()) {
                    ...
                    if (ut.each(b, function(n, s) {
                        ...
                        if (m) R = e.left + (e.right - e.left) / 2, O = "bottom" === f ? e.bottom - B - A.bottom : e.top + B + A.top;
                            ...
                            var N = "left" === f;
                            R = N ? e.left + B + A.top : e.right - B - A.top, O = e.top + (e.bottom - e.top) / 2, z = N ? 2 * Math.PI : .5 * Math.PI
                        }
                       // N.G.Carlon(FDC) Modify left scaleLabel labelString text orientation as vertically upright(change z = N ? 2* Math.PI : .5 * Math.PI)
                       if (!m ){
                    	   var pos = O-50;
                    	   console.log("R O" + R + " == " + O)
                    	   for (var i=0 ; i<c.labelString.length ; i++){
                    		   s.save(), s.translate(R, pos+=20), s.rotate(z), s.textAlign = "center", s.textBaseline = "middle", s.fillStyle = P, s.font = I.string, s.fillText(c.labelString.charAt(i), 0, 0), s.restore()
                    	   }
                       }else{
